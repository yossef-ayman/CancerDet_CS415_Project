...
export default function ProfileScreen() {
  ...
  // Generic "No Image" Silhouette (Facebook Style)
  ...
  return (
    <ThemedView style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {/* Profile Header (Avatar Only, No Cover) */}
        <View style={styles.profileHeader}>
          <Image
            source={{ uri: userProfile?.photoURL || DEFAULT_AVATAR }}
            style={styles.avatar}
            contentFit="cover"
          />
          
          <ThemedText type="title" style={styles.displayName}>{userProfile?.displayName || 'User'}</ThemedText>
...
const styles = StyleSheet.create({
  scrollContent: {
      paddingTop: 60, // Space for status bar / top area
  },
  profileHeader: {
    alignItems: 'center',
    marginBottom: 30,
  ...
  section: {
    marginBottom: 24,
    paddingHorizontal: 16, // Add padding back since we lost ParallaxScrollView container
  },
  sectionTitle: {
    marginBottom: 10,
    marginLeft: 4,
  ...
  logoutButton: {
    marginTop: 10,
    marginBottom: 40,
    marginHorizontal: 16, // Add margin since we lost container
    alignItems: 'center',
    padding: 16,
    borderRadius: 12,
